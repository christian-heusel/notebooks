...
var _ = Describe("StorageClasses Handler", func() {
	...
	Context("when storage classes exist", Serial, func() {
		...
		It("should retrieve storage classes successfully with a valid namespaceFilter query parameter", func() {
			By("creating the HTTP request with namespaceFilter query parameter")
			req, err := http.NewRequest(http.MethodGet, constants.AllStorageClassesPath+"?namespaceFilter=kubeflow-user-example-com", http.NoBody)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should return 422 for an invalid namespaceFilter query parameter", func() {
			...
			req, err := http.NewRequest(http.MethodGet, constants.AllStorageClassesPath+"?namespaceFilter=INVALID_NS!!!", http.NoBody)
			Expect(err).NotTo(HaveOccurred())
