...
var _ = Describe("controller", Ordered, func() {
	...
	BeforeAll(func() {
		...
		cmd = exec.Command("kubectl", "apply",
			"-k", filepath.Join(projectDir, "test/e2e/manifests/common"),
	...
	AfterAll(func() {
		...
		cmd := exec.Command("kubectl", "delete", "-f",
			filepath.Join(projectDir, "test/e2e/manifests/jupyterlab_v1beta1_workspace.yaml"),
			"-n", workspaceNamespace,
			"--wait",
			fmt.Sprintf("--timeout=%s", timeout),
		)
		...
		cmd = exec.Command("kubectl", "delete",
			"-f", filepath.Join(projectDir, "test/e2e/manifests/jupyterlab_v1beta1_workspacekind.yaml"),
		)
		_, _ = utils.Run(cmd)
	...
	Context("Operator", func() {
		...
		It("should run successfully", func() {
			...
			createWorkspaceSample := func() error {
				cmd := exec.Command("kubectl", "apply",
					"-f", filepath.Join(projectDir, "test/e2e/manifests/jupyterlab_v1beta1_workspace.yaml"),
					"-n", workspaceNamespace,
				)
				_, err := utils.Run(cmd)
				return err
